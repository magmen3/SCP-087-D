...
;-----------------      Setting up pause screen      ---------------------;
	...
	If GameStarted = True And AmbientTimer = 0 Then
		...
		EmitSound(AmbientSFX(Rand(1,30)),SoundEmitter)
	EndIf
...
Function CreateGlimpses()
	...
	For i = 8 To 100
		If (FloorActions(i) = 0) Then;And Rand(0,2)=0 Then 
			...
			CreateGlimpse(Floor(Rand(StartX,EndX))+0.5, FloorY,Floor(Rand(FloorZ,FloorZ+5.0))+0.5, GlimpseTextures(Rand(0,5)))
		EndIf 
	Next
		...
		Select FloorActions(PlayerFloor)
			...
			Case ACT_173_2
				...
				If FloorTimer(PlayerFloor) = 62 Then ;Button1 end of move
					EmitSound(AmbientSFX(1), door)
					EmitSound(AmbientSFX(9), door)
			...
			Case ACT_LOCK
				...
				If FloorTimer(PlayerFloor) > 2 And FloorTimer(PlayerFloor) < 250 Then
					...
					If FloorTimer(PlayerFloor) = 180 Then EmitSound (AmbientSFX(1),door)
					If FloorTimer(PlayerFloor) = 340 Then EmitSound (AmbientSFX(16),door)
				...
				If FloorTimer(PlayerFloor) >= 250 Then
					If FloorTimer(PlayerFloor) = 410 Then EmitSound (AmbientSFX(18),door)
					If FloorTimer(PlayerFloor) = 510
						PlaySound (AmbientSFX(17))	
					EndIf		
					If FloorTimer(PlayerFloor) = 610 Then 
						EmitSound (AmbientSFX(2),SoundEmitter)		
					EndIf		
					If FloorTimer(PlayerFloor) = 720
						PlaySound (AmbientSFX(18))	
					EndIf
